  ...
  >
    ...
    >
      ...
      <el-form-item label="性别" prop="gender">
        <el-select v-model="form.gender" placeholder="请选择性别" style="width: 100%" @change="logGenderChange">
...
// 调试：记录性别变化
const logGenderChange = (value: string) => {
...
watch(() => props.modelValue, (val) => {
  visible.value = val
  if (val) {
    console.log('🎯 对话框打开，传入的成员数据:', props.member)
    if (props.isEdit && props.member) {
      // Edit mode - fill form with member data
      Object.assign(form, {
      ...
      console.log('📝 表单填充后的数据:', JSON.parse(JSON.stringify(form)))
    } else {
      // Add mode - reset form
      resetForm()
    }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.isEdit && props.member) {
      console.log('🔄 开始更新成员，ID:', props.member.id)
