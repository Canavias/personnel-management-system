...
export const useMemberStore = defineStore('member', () => {
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      console.log('✅ 成员数据加载成功，总数:', response.data.length)
      
      // 详细检查每个成员的性别和更新时间
      response.data.forEach((member: Member, index: number) => {
        console.log(`成员 ${index + 1}: ${member.name} | 性别: ${member.gender} | ID: ${member.id} | 更新时间: ${member.updated_at}`)
      })
      
      // 特别检查ID为12的成员
      const member12 = response.data.find((m: Member) => m.id === 12)
      if (member12) {
        console.log('🔍 特别检查ID=12的成员:', member12)
      }
